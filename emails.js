...
// processCompanyData();
//read from companies_with_emails.json
const companyData = JSON.parse(
  fs.readFileSync("companies_with_emails.json", "utf-8")
);

//read from emails.json
const emailData = JSON.parse(fs.readFileSync("emails.json", "utf-8"));

//output emails
const outputEmails = companyData.map((company) => {
  return {
    name: company.name,
    email: company.email
  };
...
fs.writeFileSync("emails.json", JSON.stringify(outputEmails, null, 2));

console.log("Emails have been saved to emails.json");
